...
- **2026-08-11 (via um comparador com coletor client-side - trocar chave-embutida por handshake
  por-instalação, e a race da classe 1 no SQLite):** Quando a dívida de 2026-08-10 (a "chave" que viaja
  no artefato distribuído não é auth) é paga com um **registro aberto por instalação** (`POST /installs`
  devolve `{id, token}`, token guardado só como **sha256**, e o endpoint de ingestão passa a exigir esse
  token), a superfície NOVA a auditar não é o token em si (esse desenho está certo: credencial opaca por
  instalação, hash no banco, alvo do POST é UUID não-enumerável) - é o **par de controles anti-abuso
  check-then-act** que quase sempre nasce junto: (a) **teto de registros por IP/dia** (`SELECT COUNT ->
  INSERT`) e (b) **rate limit por instalação** (`ler contador -> comparar -> UPDATE`). Os dois são a
  **classe 1** escondida num endpoint sem cara de "dinheiro/quota", e os dois **furaram** sob `Promise.all`
  de processo único (40 registros do mesmo IP com teto 20 -> 40×200; 120 posts com 1 token e teto 80 ->
  120×200). **No SQLite não há advisory lock; a atomicidade é `BEGIN IMMEDIATE`** (pega o lock de escrita
  já na ABERTURA da transação, antes do SELECT) - e a armadilha específica do módulo `sqlite3` do Python é
  que ele **só auto-começa transação antes de um DML, não antes de um SELECT**, então o `SELECT COUNT` do
  teto roda SEM lock e a janela de corrida existe mesmo "dentro de um `with conn`". Fix: um helper
  `db_write()` com `isolation_level=None` + `BEGIN IMMEDIATE`/`COMMIT` manual (busy_timeout generoso p/ a
  rajada serializar). **▶ Testar ao vivo:** `Promise.all` de N >> teto contra o backend LOCAL (isola o
  teto da app do rate-limit do nginx) e **conte as linhas no DB**, não só os status; a prova de fix é
  N-teto × 429 e exatamente `teto` linhas. **Trave com teste concorrente de verdade** (`ThreadPoolExecutor`
  contra o `TestClient` - endpoints **sync** do FastAPI rodam no threadpool, então concorrem de fato; um
  `TestClient` sequencial NÃO pega a race) e mutation-check (voltar `db_write`->`db` deixa só os testes de
  race vermelhos). **Classe 12 de brinde:** o teto por IP só vale se o IP não for forjável - confirme que o
  nginx faz `proxy_set_header X-Real-IP $remote_addr` (sobrescreve o header do cliente) mandando um
