...
| `ai-delegate` | `/ai-delegate` | **Orquestra IAs gratuitas para economizar tokens do Claude**: delega tarefas braçais delimitadas (boilerplate, testes de função existente, docstrings, i18n, commit msgs, resumos) para Ollama local (qwen3.5:9b/4b, qwen2.5-coder:14b), Groq (gpt-oss-120b, ~470 tok/s, não treina, 200K tokens/dia) e Gemini Flash (ctx 1M, **treina** - só código não sensível). Claude escreve a spec → worker gera texto → Claude revisa e aplica (worker nunca edita arquivo). Inclui `scripts/ai.sh` e integra o **Cline CLI** (`cline -y`) como executor headless. Setup por máquina: Ollama + modelos + keys em `~/.config/ai-workers/` (fora do repo). |
...
| `ghostwriter` | `/ghostwriter` | **Escreve qualquer texto em nome do Gustavo com a voz dele** (Sobre do LinkedIn, bio de app de namoro, apresentações, posts, recomendações), contra um `voice.md` versionado com registro por contexto, marcadores de estilo, antipadrões e corpus de textos aprovados, que cresce a cada uso. Use em "escreva/refaça minha descrição/bio", "não parece eu", "está formal/vendedor demais", e SEMPRE antes de gerar texto publicado em nome dele. Par da `linkedin` (uma escreve, a outra publica no perfil). |
| `unity-mcp-skill` | `/unity-mcp-skill` | Orquestra o Unity Editor via MCP for Unity (GameObjects, scripts, cenas, testes, automação do editor): boas práticas, schemas das tools e padrões de workflow. Só é útil em máquina com o MCP for Unity conectado. |
...
| `chrome-web-store` | `/cws` | Publica extensões do Chrome na Chrome Web Store via API oficial (upload de pacote .zip + publish/enviar pra revisão), como a `google-play` faz com apps. OAuth próprio do dono (client "Aplicativo da Web" + refresh token escopo `chromewebstore`); credenciais em `~/.config/bobagi-google/` (`cws-client.json`/`cws-token.json`/`cws-items.json`), setup único em `chrome-web-store/SETUP.md`. Limites: não edita descrição longa/screenshots da ficha e a publicação passa por revisão. Extensão padrão - Farolivro. |
